...
- Kept the overall prep posture proof-first when metric evidence is thin or missing, even if there are no narrative claims, because board scrutiny can start from the KPI backup itself.
...
- Ignore placeholder open-risk rows such as `TBD` and `N/A` so import scaffolding does not create phantom risk-register work.
